...
const FLAGS := {
	...
	"--record": "record this session to user://sessions (run_game.bat passes it)",
	"--no-record": "do not record this session, whatever the setting says",
	"--replay=": "<file>      play a recorded session back as puppetry",
	"--replay-test": "diagnostic: a scripted session, recorded, then quit",
	...
	"--mob-pitch=": "<rad>    with --mob-shot: look up (+) or down (-0.15)",
	"--look=": "<yaw,pitch>    with --shots: one shot from this pose (radians)",
	"--item-test": "diagnostic: equip known item properties, print what they do, quit",
	"--skill-tips": "diagnostic: print every skill's tooltip numbers at levels 1, 10 and 20, quit",
